...
summary: TypeScript only helps when you let it. The habits that turn type checking off include `as`, `any`, unvalidated boundaries, and strict flags left disabled.
tags: ['typescript', 'type-safety', 'zod', 'runtime-validation', 'tsconfig']
---

Most TypeScript bugs I've shipped came from lines where I told the compiler to stop checking, and it obliged.
...
TypeScript catches mistakes when you make them, but only the ones you let it catch. The patterns that turn checking off don't always look like an opt-out. Most of them pass code review because they look like fine code.
...
The double assertion, `as unknown as T`, launders the value through `unknown` to bypass the check the compiler refused to skip. If you write it, you should be able to defend the cast. Nothing else will.
...
Three other types cause similar gaps:
...
TypeScript cannot verify data as it crosses into your app from outside. The unsafe code still looks normal:
...
`Body.json()` returns `Promise<any>`, so `: User` is a type assertion in costume. You are telling TypeScript what's there; TypeScript is not checking it. The network is outside the language. The same hole exists everywhere data crosses in:
...
Runtime validation checks the shape at the boundary. [Zod](https://zod.dev/), [Valibot](https://valibot.dev/), [ArkType](https://arktype.io/), and [io-ts](https://gcanti.github.io/io-ts/) all provide schemas for that check:
...
The `User` type derives from the schema instead of being declared in parallel. That leaves one source of truth and no drift. `parse` checks at runtime: if the shape is wrong, it throws with a structured error you can log, retry, or show to the user.

For inputs that can legitimately fail (form data, third-party webhooks, anything user-controlled), `safeParse` returns a discriminated union: `{ success: true, data: T }` or `{ success: false, error: ZodError }`. You branch on the failure instead of catching it.
...
You'd expect `key` to be `'theme' | 'density'`. It isn't, because at runtime the object might have extra keys the type doesn't know about, including anything tacked on by a JSON parse, a prototype, or a later mutation. The honest answer is "I don't know all the keys."
...
You want `includes` to check whether an arbitrary string is one of the valid ones. TypeScript blocks it because the array's element type is the narrow union, and `includes` insists the argument be assignable to that union. You don't have an `'active' | 'pending' | 'paused'`; that's the question you're trying to answer.
...
By default, indexing an array gives you back the element type. TypeScript trusts that you know the index is in range. The runtime makes no such promise.

Two flags that aren't part of `strict` close this gap:
...
With `noUncheckedIndexedAccess`, `arr[5]` returns `T | undefined`, and the compiler forces you to handle the empty case at every index, every `Map.get`, and every `Record` lookup. It's annoying for two days. After that, off-by-one bugs and missing-key bugs you used to ship in production start getting caught at the keyboard.
...
Those two values look identical at the type level but behave differently at runtime. `'name' in user` is `true` for the version with `undefined` and `false` for the one without. With the flag on, an optional property allows the key to be missing but rejects an explicit `undefined` unless the type includes it.

Both flags ship turned off. Turn them on when you want indexed access and optional properties to describe the values the runtime can return.
...
Give each kind of string its own type by attaching a tag, a marker that only exists in the type system:
...
A `UserId` is "a string, plus a marker saying it's a UserId." The marker is invisible at runtime, so there is nothing to serialize and no runtime overhead. TypeScript treats `UserId` and `OrderId` as different types because their markers don't match, even though both are strings underneath.
...
None of these changes needs a sprint. Most fit in a single PR, a tsconfig change, or a habit you build by writing one line differently. If you only do one this week, turn on `noUncheckedIndexedAccess` and fix what it surfaces. It will find bugs you've been shipping for years.
