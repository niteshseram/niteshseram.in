...
Both page requests were valid, but each ran against a different version of the list. Something was inserted, deleted, or reordered between them. The page 2 request identified a position in the current result, not the item where page one ended.
...
An offset remembers a position in the result. A cursor carries the fields that placed the last record in the ordered list. Keyset pagination is the database query that continues from those fields.
...
The query followed the requested offset and counted from the start of the list as it looked at that moment.
...
The first request uses offset `0`; the second uses offset `20`. The second query runs against the current result and discards its first 20 rows. It has no record of the last post returned by page one.
...
This partial index fits a query that always asks for published posts. Check the real query with `EXPLAIN ANALYZE` because the best index depends on the data and workload.
...
A cursor stops changes above the boundary from shifting the next page, but later pages can still read a different version of the list.
...
The live list can change while every page in the paging session keeps reading the view that page one used.
