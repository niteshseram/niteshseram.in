...
<Image
  ...
  height={563}
/>
In the above image, you can see that a variable `abc` is declared but not used
anywhere in that file. But, the variable `abc` is not underline which means no
issue was found.
<Image
...
<Image
  ...
  height={555}
/>
In the above image, you can see that the variable `abc` is now underline and is
being caught by ESLint.
<Image
  alt="ESLint in Console"
