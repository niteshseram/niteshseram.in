...
export const metadata: Metadata = {
  title: 'Design system',
  description:
    'A living reference for the tokens, typography, and components used across the site.',
...
const VERTICAL_SCROLL_ITEMS = Array.from({ length: 30 }, (_, index) => ({
  id: `vertical-item-${index}`,
  label: `Item ${index + 1}`,
}));

const HORIZONTAL_SCROLL_TAGS = Array.from({ length: 20 }, (_, index) => ({
  id: `horizontal-tag-${index}`,
  label: `Tag ${index + 1}`,
}));

const BOTH_SCROLL_ROWS = Array.from({ length: 20 }, (_, index) => ({
  id: `both-row-${index}`,
  label: `Row ${index + 1}: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.`,
}));
...
function ColorsSection() {
  return (
    <Section eyebrow="01" title="Colors">
      ...
      <div className={cn('grid grid-cols-2 gap-3 sm:grid-cols-4', 'mt-3 mb-8')}>
          ...
          <div key={variant}>
            ...
            <DemoRow label="Sizes">
              {buttonSizes.map((size) => (
                <Button key={size} label={size} size={size} variant={variant} />
              ))}
              <Button
                disabled={true}
                label="disabled"
          ...
          <DemoRow>
            ...
            >
              <ul className={cn('flex flex-col gap-y-1.5', 'text-sm')}>
                {VERTICAL_SCROLL_ITEMS.map((item) => (
          ...
          <DemoRow>
            ...
            >
              <div className="flex w-max gap-x-2">
                {HORIZONTAL_SCROLL_TAGS.map((tag) => (
                  <span
                    key={tag.id}
                    className={cn(
